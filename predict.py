...
layers_dims = [11, 3, 5, 5, 3, 3, 3, 1]
parameters, b_par, v_mu, v_sigma = adam_model(X_train, y_train, layers_dims, optimizer='momentum', decay_rate=0.002,
...
from pred_func import predict
def pred(X, Y, parameters):
    y_pred = predict(X, parameters, b_par, v_mu, v_sigma)
    return y_pred
...
accuracy_test = cm_test[1][1] / (cm_test[0][1] + cm_test[1][1])
accuracy_test = str(accuracy_test*100)+'%'

## Validation Accuracy
y_pred = pred(X_val, y_val, parameters)
y_pred[y_pred<=0.5] = 0
y_pred[y_pred>0.5] = 1
...
for i in range(len(y_pred[0])):
    if (int(y_pred[0][i]) == int(y_val[0][i])):
        n = n + 1
    if (int(y_pred[0][i]) != int(y_val[0][i]) and int(y_pred[0][i]==0)):
        cm_val[0][1] = cm_val[0][1] + 1
    if (int(y_pred[0][i]) != int(y_val[0][i]) and int(y_pred[0][i]==1)):
...
accuracy_val = cm_val[1][1] / (cm_val[0][1] + cm_val[1][1])
...
accuracy_train = cm_train[1][1] / (cm_train[0][1] + cm_train[1][1])
