...
def lin_act_forward(A_prev, W, b, b_par, gamma, beta, D, keep_prob, activation):
    """
...
def L_model_forward(X, parameters, b_par, keep_prob=0.5):
    caches = []
    A = X
...
#        print (b_par["gamma"+str(l)].shape)
        ...
        A, cache, b_par, D = lin_act_forward(A_prev, parameters["W" + str(l)], parameters["b" + str(l)], 
                                        b_par, gamma, beta, D, keep_prob, activation='relu')
        if b_par['mode'] == 'train':
            caches.append(cache)
...
#    print (b_par["gamma"+str(L)].shape)
    ...
    AL, cache, b_par, D = lin_act_forward(A, parameters["W" + str(L)], parameters["b" + str(L)],
                                                     b_par, gamma, beta, D, keep_prob, activation='sigmoid')
